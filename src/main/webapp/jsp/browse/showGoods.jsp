  		...
  		<% 
  			...
  			if(totalPages >= 1)
  			{
  			   ...
  			   for(int i=1,j=goods.getPageSize();i<=j&&flag;i++)
  			   {
  			       ...
  			       String made = rowSet.getString(4);
  			     ...
  			     Products = ID+","+name+","+made+","+price+","+number;//尾缀#，便于计算购物车价格
  			   Products = Products.replaceAll("\\p{Blank}","");
  			       
  			       String shopCarButton = "<form action='hjy.goods/PutGoodsToCar' method='post'>"+
  			                       "<input type='hidden' name='GoodsCar' value="+Products+">"+
  			                       "<input type='submit' value='加入购物车'></form>";
  			       String detail = "<form action='jsp/browse/showDetail.jsp' method='post'>"+
  			                       "<input type='hidden' name='detail' value="+Products+">"+
