...
public class HandleguanliLogin extends HttpServlet 
{
	private static final long serialVersionUID = 1L; //设置序列号
	public HandleguanliLogin()
	{
		super();
	}
			...
			throws ServletException, IOException
	{
		...
		String sql = "SELECT * FROM `mydatabase`.`guanli` WHERE `account`=? AND `password`=?";
		try
		{
	...
	public void handleCookies(HttpServletRequest request, HttpServletResponse response, 
	    ...
	    if ("isCookie".equals(isCookie)) { // 用户选择了记住密码
	        ...
	        if (name != null) {
	            ...
	            Cookie nameCookie = new Cookie("name", Username); // 设置与登陆时的name对应的键值对
	        ...
	        if (cookies != null && cookies.length > 0) {
	            for (Cookie c : cookies) {
	                if ("name".equals(c.getName()) || "pass".equals(c.getName())) {
			...
			HttpServletResponse response,String backNews)
	{
		try
		{
			...
			out.print("返回"+"<a href=/hjy1/jsp/join/guanlilogin.jsp>登陆界面</a>");
		} catch (IOException e)
		{
			e.printStackTrace();
		}
